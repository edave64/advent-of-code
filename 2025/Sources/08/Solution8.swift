...
struct Solution8: Solution {
  ...
  func partA(input: String) throws -> String {
    ...
    // And it's an array since two points can have the same distance from each other
    ...
    let sortedDistances = distances.sorted(by: { $0.key < $1.key })

    // Connect the junction boxes and manage the clusters
    for (_, pointPairs) in sortedDistances {
